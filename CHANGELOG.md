...
- **Thread Safety Improvement**: Janitor now uses `chan struct{}` instead of `chan bool` for better memory efficiency and Go idioms

### Fixed
- **ShardedCache Load Bug**: Fixed issue where items were not properly re-hashed to correct shards during deserialization
